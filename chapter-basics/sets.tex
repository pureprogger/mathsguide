...
We introduce the axioms of \emph{Zermelo-Fraenkel} (ZF) set theory, a standard foundation for modern mathematics. Later we will extend the Zermelo-Fraenkel axioms with the \emph{axiom of choice} to form the ZFC set theory. We will not explore the meta-mathematical properties of these axioms -- such as their logical independence from one another -- but will instead focus on how they are used to construct sets with formal rigor.
...
\begin{axiom}[Extensionality]
    ...
    We call this axiom the \emph{axiom of extensionality}.
\end{axiom}
...
\begin{proof}
    ...
    and thus $x\in A\Leftrightarrow x\in B$. Because $x$ is an arbitrary set, $x\in A\Leftrightarrow x\in B$ holds for every set $x$. Hence $A=B$ by the axiom of extensionality.
\end{proof}

The existence of an extension of a property $\varphi(x)$ has to be axiomatized for specific types of properties. The following axiom guarantees the existence of a set that contains no elements, hence we assume there exists a so-called empty set $\emptyset$ as an extension of the property $\varphi(x)\equiv x\notin \emptyset$:

\begin{axiom}[Empty set]
    There exists a set $\emptyset$ such that for all sets $x$, $x\in\emptyset$ does not hold, or formally: \begin{align}\forall x\colon x\notin\emptyset.\end{align} We call $\emptyset$ the \emph{empty set}.
\end{axiom}

\begin{remarks}
    \begin{remarkenumerate}
        \item The empty set is an extension of the property $\varphi(x)\equiv x\notin\emptyset$. Hence by theorem \ref{theorem:sets:extension_unique} the empty set is unique. More precisely, if $A$ is a set with $\forall x\colon x\notin A$ then $A=\emptyset$ holds.
...
\begin{axiom}[Pairing]
    ...
    We call $\{A,B\}$ the \emph{pair} of $A$ and $B$.
\end{axiom}
...
\begin{axiom}[Powerset]
    ...
    We call $\powerset(A)$ the \emph{powerset} of $A$.
\end{axiom}
...
\begin{axiom}[Specification]
    For every set $A$ and every property $\varphi(x)$, there exists a set $\setdef{x\in A\given\varphi(x)}$ such that for all sets $y$, $y\in\setdef{x\in A\given\varphi(x)}$ holds if and only if $y\in A$ and $\varphi(y)$ holds. More formally:
    \begin{align}\forall y\colon y\in\setdef{x\in A\given\varphi(x)}\Leftrightarrow y\in A\land \varphi(y).\end{align}
    We call $\setdef{x\in A\given\varphi(x)}$ the \emph{subset of $A$ defined by the property $\varphi$}.
\end{axiom}
...
\begin{axiom}[Union]
    ...
    We call $\bigcup A$ the \emph{union} of $A$.
\end{axiom}
...
\begin{axiom}[Foundation]
    ...
    A set $F\in S$ with the property $\forall x\in S\colon x\notin F$ is called a \emph{foundation} of $S$.
\end{axiom}

\begin{theorem}\label{theorem:sets:set_not_in_set}
...
\begin{theorem}\label{theorem:sets:element_of_antisymmetric}
