...
\begin{examples}
    \begin{exampleenumerate}
        ...
        \end{miniproof}
        \item\label{example:mappings:singleton_mapping} Let $X$ be a set, then the \emph{singleton mapping} defined by
        \begin{align}s\colon X\to\powerset(X),\qquad x\mapsto\{x\}\end{align}
        is an injection.
    \end{exampleenumerate}
\end{examples}
...
\begin{axiom}[Axiom of Choice]\label{axiom:mappings:choice}
    ...
    so that $f(F)\in F$ holds for all $F\in\mathcal F$. We write \axiomsymbol{AC} for the \emph{Axiom of Choice}.
\end{axiom}
...
\begin{remarks}
    \begin{remarkenumerate}
        ...
        \item Let $X$ be a set, then there exists a choice mapping $f\colon\powerset(X)\setminus\{\emptyset\}\to X$ which assings every non-empty subset $A\subseteq X$ to an element $f(A)\in A$.
    \end{remarkenumerate}
\end{remarks}
